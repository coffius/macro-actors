...
class RuntimeCompiler(targetDir: Option[File] = None) {
...
//      val strToCompile = wrapCodeInClass(className, code)
      val strToCompile = code
      val sourceFiles = List(new BatchSourceFile("(inline)", strToCompile))
  ...
  /** Compiles the source string into the class loader and
    ...
    * @return
    */
  def eval[T](className: String, code: String): T = {
    val className = classNameForCode(code)
    val cls = compile(className, code)
